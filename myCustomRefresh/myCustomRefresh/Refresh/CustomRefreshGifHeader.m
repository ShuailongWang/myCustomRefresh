...
- (void)startNorAnimation{
    NSArray *imageArr = @[[UIImage imageNamed:@"CustomRefreshAnimation01"],
                          [UIImage imageNamed:@"CustomRefreshAnimation02"],
                          [UIImage imageNamed:@"CustomRefreshAnimation03"],
                          [UIImage imageNamed:@"CustomRefreshAnimation04"]];
    [self setImages:imageArr duration:0.5 forState:MJRefreshStateIdle];
    [self setImages:imageArr duration:0.5 forState:MJRefreshStatePulling];
    [self setImages:imageArr duration:0.5 forState:MJRefreshStateRefreshing];
    [self setImages:imageArr duration:0.5 forState:MJRefreshStateWillRefresh];
}

#pragma mark - 懒加载
- (UIImageView *)topImage {
    if (!_topImage) {
        [self addSubview:_topImage =[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"CustomRefresh_Top"]]];
        _topImage.frame = CGRectMake(0, -360, KScreen_Width, 360);
...
- (UIView *)bgView {
    if (!_bgView) {
        ...
        for (int i = 0; i < 2; i ++) {
            UIImageView *image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"CustomRefreshAnimation_bg"]];
